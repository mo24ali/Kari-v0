        ...
        <div class="w-full max-w-5xl">
            ...
            <div class=" rounded-3xl p-8 shadow-2xl border border-gray-100 dark:border-gray-700 transition-all">
                <form method="get" action="/">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div>
                            <label class="block text-sm font-bold text-gray-800 dark:text-gray-200 mb-2 flex items-center">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" class="w-full pl-12 pr-4 py-4 bg-gray-50 dark:bg-gray-900 border-2 border-gray-100 dark:border-gray-700 rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/20 dark:text-white transition-all outline-none" 
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select class="w-full pl-12 pr-4 py-4 bg-gray-50 dark:bg-gray-900 border-2 border-gray-100 dark:border-gray-700 rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/20 dark:text-white transition-all appearance-none outline-none">
                                ...
                                <i class="fas fa-user-friends absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"></i>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center gap-6">
                        <div class="flex space-x-3 overflow-x-auto pb-2 w-full md:w-auto scrollbar-hide">
                            <button type="button" class="category-tag active px-4 py-2 rounded-full border border-primary/20 bg-primary/5 text-primary text-sm font-bold whitespace-nowrap flex items-center">
                                <i class="fas fa-umbrella-beach mr-2"></i> Plage
                            </button>
                            <button type="button" class="category-tag px-4 py-2 rounded-full border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 text-sm font-bold whitespace-nowrap flex items-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <button type="submit" class="w-full md:w-auto bg-primary text-white font-black py-4 px-10 rounded-2xl hover:bg-primary-dark shadow-xl shadow-primary/30 transition-all transform hover:scale-105 flex items-center justify-center group uppercase tracking-widest text-sm">
            ...
            <div class="flex flex-wrap justify-center md:justify-start gap-8 mt-10">
                <div class="flex items-center">
                    <div class="w-2.5 h-2.5 bg-green-500 rounded-full mr-3 animate-pulse shadow-[0_0_10px_rgba(34,197,94,0.5)]"></div>
                    <span class="text-gray-600 dark:text-gray-300 text-sm font-medium">
                        <span class="font-bold text-gray-900 dark:text-white">1,248</span> logements disponibles
                    </span>
                </div>
                <div class="flex items-center">
                    <div class="w-2.5 h-2.5 bg-yellow-500 rounded-full mr-3 shadow-[0_0_10px_rgba(234,179,8,0.5)]"></div>
                    <span class="text-gray-600 dark:text-gray-300 text-sm font-medium">
...
<div class="container mx-auto px-4 py-12">
    ...
    <?php if (isset($_SESSION['success'])): ?>
        <div class="mb-8 flex items-center gap-3 rounded-2xl border border-green-200 dark:border-green-900/50 bg-green-50/50 dark:bg-green-900/20 px-6 py-4 font-medium text-green-800 dark:text-green-300 backdrop-blur-sm transition-all animate-fade-in">
    ...
    <div class="mb-12 flex flex-col sm:flex-row items-end justify-between gap-6">
        <div class="max-w-xl">
            ...
            <p class="text-gray-500 dark:text-gray-400 font-medium">Parcourez nos meilleures offres sélectionnées pour vous.</p>
        </div>
        <?php if ($isHost): ?>
    ...
    <?php else: ?>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-12">
            <?php foreach ($logements as $logement): ?>
                <div class="group flex flex-col h-full bg-white dark:bg-gray-900 rounded-[2.5rem] overflow-hidden border border-transparent dark:border-gray-800 hover:shadow-2xl dark:hover:shadow-black/50 transition-all duration-500 hover:-translate-y-1">
                    ...
                    <div class="relative aspect-[4/5] overflow-hidden rounded-[2rem] m-2.5">
                        <div class="airbnb-img-carousel h-full w-full relative">
                            ...
                            <?php if (!empty($carouselImages)): ?>
                                ...
                                <div class="absolute inset-0 flex items-center justify-between px-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                                    <button class="airbnb-carousel-btn left pointer-events-auto w-8 h-8 rounded-full bg-white/90 dark:bg-gray-800/90 text-gray-800 dark:text-white flex items-center justify-center shadow-md hover:scale-110 transition-transform">
                                        <i class="fas fa-chevron-left text-xs"></i>
                                    </button>
                                    <button class="airbnb-carousel-btn right pointer-events-auto w-8 h-8 rounded-full bg-white/90 dark:bg-gray-800/90 text-gray-800 dark:text-white flex items-center justify-center shadow-md hover:scale-110 transition-transform">
                        ...
                        <?php if ($userRole === 'host' && $logement['id_owner'] == ($_SESSION['user_id'] ?? null)) : ?>
                            <span class="absolute top-4 left-4 bg-primary text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">
                                Votre annonce
                            </span>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="px-5 pb-6 flex flex-col flex-grow">
                        ...
                        <div class="mt-auto space-y-4">
                            <div class="flex items-baseline gap-1 text-gray-900 dark:text-white">
                                <span class="text-2xl font-black"><?php echo number_format($logement['price'], 0, ',', ' '); ?> €</span>
                            ...
                            <?php if ($userRole && $userRole !== 'host' && $logement['id_owner'] != ($_SESSION['user_id'] ?? 0)): ?>
                                <form method="POST" action="/reservation/create" class="space-y-3">
                                    ...
                                    <div class="grid grid-cols-2 gap-px bg-gray-200 dark:bg-gray-700 rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
                                        <div class="bg-white dark:bg-gray-800 p-3">
                                            <label class="block text-[9px] uppercase font-black text-gray-400 mb-1">Arrivée</label>
                                            <input type="date" name="start_date" required min="<?php echo date('Y-m-d'); ?>" class="w-full bg-transparent text-xs font-bold text-gray-900 dark:text-white outline-none">
                                        </div>
                                        <div class="bg-white dark:bg-gray-800 p-3">
                                            <label class="block text-[9px] uppercase font-black text-gray-400 mb-1">Départ</label>
                                            <input type="date" name="end_date" required min="<?php echo date('Y-m-d'); ?>" class="w-full bg-transparent text-xs font-bold text-gray-900 dark:text-white outline-none">
                                        </div>
                                    </div>
                                    <button type="submit" class="w-full bg-primary hover:bg-primary-dark text-white py-3.5 rounded-2xl font-black transition-all shadow-lg shadow-primary/20 active:scale-95 uppercase text-xs tracking-widest">
                                        Réserver le séjour
                                    </button>
                                </form>
                            <?php elseif (!$userRole): ?>
                                <a href="/login" class="block text-center w-full bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-white py-3 rounded-2xl font-bold hover:bg-primary hover:text-white transition-all text-sm">
...
<style>
    ...
    .airbnb-img-carousel img { display: none; }
    .airbnb-img-carousel img.active { display: block; }
</style>
